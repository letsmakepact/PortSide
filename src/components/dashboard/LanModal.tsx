...
import { Smartphone, Globe, Copy, Check, ExternalLink, Sparkles, Tv, ShieldCheck } from "lucide-react";
...
export function LanModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const isLanDashboardSelected = selectedService === "__lan__";
  ...
  const wildcardSvcUrl = activeHostname ? `http://${activeHostname}.${lanIp}.nip.io${portSuffix}` : portalUrl;
  ...
  const portsideRedirectUrl = activeHostname ? `https://${vanityDomain}/s/${activeHostname}` : `https://${vanityDomain}/lan`;
        ...
        <div className="mt-5 grid gap-6 sm:grid-cols-2">
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-800/80 bg-[#080e1a] p-5 text-center shadow-inner">
            ...
            <div className="mt-3.5 max-w-[240px]">
              <p className="font-mono text-[11px] text-sky-400 truncate font-semibold" title={lanData?.qrTarget}>
                {lanData?.qrTarget ? lanData.qrTarget.split("?")[0] : "Connecting..."}
              </p>
              <p className="mt-1 text-[11px] text-slate-400 leading-tight">
                {connectionMode === "tunnel"
                  ? "Global 5G tunnel URL. Accessible on any cellular data or external network."
          ...
          <div className="flex flex-col justify-between space-y-4">
            ...
            {/* Standard Wi-Fi Redirect Details (Free & Direct) */}
            {connectionMode === "lan" && (
              <div className="space-y-2.5 rounded-xl border border-slate-800 bg-slate-900/60 p-3 text-xs">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-emerald-400">Direct Project Wi-Fi URL</span>
                    <button
                      type="button"
                  ...
                  <p className="font-mono text-slate-300 break-all mt-0.5">{directSvcUrl}</p>
                </div>

                <div>
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-slate-400">Wildcard Subdomain Redirect</span>
                    <button
                      type="button"
                  ...
                  <p className="font-mono text-slate-400 break-all mt-0.5">{wildcardSvcUrl}</p>
                </div>
              </div>
            )}

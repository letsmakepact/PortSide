...
export function ServiceCard({
  ...
  return (
    <div
      className={cn(
        "group relative flex flex-col rounded-lg border p-4 transition bg-[#111827]",
        service.enabled ? "border-[#1f2937] hover:border-slate-700 hover:bg-[#131b2c]" : "border-dashed border-[#1f2937] bg-[#0d131f] opacity-70",
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <h3 className="truncate text-sm font-semibold text-white">{service.name}</h3>
            {service.favorite && (
          ...
          {(onEdit || onDelete) && (
            <div className="relative">
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded p-1 text-slate-400 transition hover:bg-[#161f30] hover:text-white"
              ...
              {menuOpen && (
                <div className="animate-fade-up absolute right-0 z-20 mt-1 w-44 overflow-hidden rounded-md border border-[#1f2937] bg-[#111827] py-1 text-xs shadow-xl">
                  ...
                  {onDelete && (
                    <MenuItem onClick={() => onDelete(service)} className="text-rose-400 hover:bg-rose-950/30">
                      Delete service
                    </MenuItem>
                  )}
                </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        <span className="inline-flex items-center rounded border border-[#1f2937] bg-[#0b0f17] px-1.5 py-0.5 font-mono text-[11px] font-medium text-slate-300">
          :{service.port}
        </span>
        {project && (
        ...
        {!compact && service.tags.map((t) => (
          <span key={t} className="rounded border border-[#1f2937] bg-[#0b0f17] px-1.5 py-0.5 text-[10px] text-slate-400 font-mono">
            {t}
          </span>
        ))}
      </div>

      <div className="mt-3 flex items-center justify-between border-t border-[#1f2937] pt-2.5 text-[10px] text-slate-400 font-mono">
        ...
        {service.lastStatus === "online" && service.lastLatencyMs != null && service.enabled && (
          <span className="font-mono text-emerald-400">{service.lastLatencyMs}ms</span>
        )}
      </div>
    </div>
  );

...
export const projects = pgTable(
  "projects",
  {
    ...
    icon: varchar("icon", { length: 32 }).default("layers").notNull(),
    category: varchar("category", { length: 64 }).default("development").notNull(),
    tags: text("tags").array().default([]).notNull(),
    repoUrl: text("repo_url").default("").notNull(),
    docsUrl: text("docs_url").default("").notNull(),
    lead: varchar("lead", { length: 120 }).default("").notNull(),
    accent: varchar("accent", { length: 32 }).default("glow").notNull(),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
